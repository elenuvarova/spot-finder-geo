...
This pipeline builds `backend/data/antwerpen.geojson`, the per-sector opportunity
layer that the SpotFinder backend serves. It reads **OpenStreetMap** (via Overpass)
and **Belgian Statbel** open data, aggregates everything onto Antwerp's statistical
sectors (real neighbourhoods), computes per-type underserved-demand opportunity
scores, and writes the GeoJSON in the exact schema the backend expects.
...
per Statbel statistical sector, with opportunity scores (`opp_cafe`, `opp_bakery`, `opp_confectionery`) plus
...
**within its own type** to produce `opp_<type>` in `0..1`; sectors with
